...
def train(args):
    class ConcatPooling(nn.Module):
        def __init__(self, num_pooling_layers):
            super(ConcatPooling, self).__init__()
            self.n_layers = num_pooling_layers
            self.output_dim = AutoConfig.from_pretrained(args.model_name).hidden_size * num_pooling_layers
    ...
    class CustomModel(nn.Module):
        def __init__(self, tokenizer):
            ...
            self.pool = ConcatPooling(num_pooling_layers=args.num_pooling_layers)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_pooling_layers", default=2, type=int)

...
class PATHS:
    ...
    valid_path = '../dataset/4k_nooverlap.csv'
...
valid = pd.read_csv(PATHS.valid_path)
train['label'] = train['score'].apply(lambda x: x-1)
valid['label'] = valid['score'].apply(lambda x: x-1)
if USE_REGRESSION: 
    train["label"] = train["label"].astype('float32')
    valid["label"] = valid["label"].astype('float32')
else: 
    train['label'] = train['label'].astype('int32')
    valid['label'] = valid['label'].astype('int32')
...
if COMPUTE_CV:
    ...
    if LOAD_FROM:
        model = AutoModelForSequenceClassification.from_pretrained( f'/root/autodl-tmp/{PATHS.model_name}_AES2_v{VER}')
    else:
    ...
    if LOAD_FROM is None:
        trainer.save_model(f'/root/autodl-tmp/{PATHS.model_name}_AES2_v{VER}')
        tokenizer.save_pretrained(f'/root/autodl-tmp/{PATHS.model_name}_AES2_v{VER}')
    ...
    valid.to_csv(f'outputs/valid_df_v{VER}.csv', index=False)

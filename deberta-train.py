...
def train_deberta(args):
    ...
    ds_train = ds_train.map(tokenize).remove_columns(
        ["essay_id", "full_text", "score", "fold", "__index_level_0__"]
    )
    ...
    scheduler = get_polynomial_decay_schedule_with_warmup(
        optimizer,
        num_warmup_steps=args.num_train_epochs *
        int(len(ds_train) * 1.0 / gpu_count / args.per_device_train_batch_size /
            args.gradient_accumulation_steps) * args.warmup_ratio,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Finetune Deberta-v3 For Sequence Classification Task"
    )
    parser.add_argument(
        "--train_file_path", default="dataset/train.csv", type=str
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_length", default=1024, type=int
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--warmp_ratio", default=0.1, type=float
    )
    parser.add_argument(
        "--save_total_limit", default=10, type=int
    )
    parser.add_argument(
        "--weight_decay", default=0.001, type=float
    )
    parser.add_argument(
        "--power", default=1.5, type=float
    )
    parser.add_argument(
        "--lr_end", default=1e-6, type=float
    )
    parser.add_argument(
        "--neftune_noise_alpha", default=0.05, type=float
    )
